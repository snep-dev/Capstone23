...
XBee2::XBee2(): _response(XBeeResponse()) {
        _pos = 0;
        _escape = false;
        _checksumTotal = 0;
        _nextFrameId = 0;
...
void XBee2::begin(Stream &serial) {
	_serial = &serial;
}

void XBee2::setSerial(Stream &serial) {
	_serial = &serial;
}

bool XBee2::available() {
...
void XBee2::write(uint8_t val) {
	_serial->write(val);
}

XBeeResponse& XBee2::getResponse() {
	return _response;
}

// TODO how to convert response to proper subclass?
void XBee2::getResponse(XBeeResponse &response) {
...
void XBee2::readPacketUntilAvailable() {
...
bool XBee2::readPacket(int timeout) {
...
void XBee2::send(XBeeRequest &request) {
	// the new new deal
...
void XBee2::sendByte(uint8_t b, bool escape) {
